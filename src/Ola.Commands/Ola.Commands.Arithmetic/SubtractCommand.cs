...
namespace Ola.Commands.Arithmetic
{
    ...
    {
        private const string RegexPattern = @"^SUBTRACT (\w+) FROM ([\w\d\.]+) AS (\w+)$";
        private readonly CommandContext _context;

        public SubtractCommand(CommandContext context)
        {
            _context = context;
        }
        ...
        public Task<int> Execute(int lineIndex, string[] commands)
        {
            ...
            if (match.Success)
            {
                ...
                if (_context.Variables.ContainsKey(variable1))
                {
                    ...
                    {
                        _context.Variables[resultVariable] = value2 - Convert.ToDecimal(int1);
                    }
                    else if (value1 is decimal dec1)
                    {
                        _context.Variables[resultVariable] = value2 - dec1;
                    }
                    else
                    {
                        Console.WriteLine($"ERROR: Cannot subtract incompatible types for variable '{variable1}' and operand '{operand2}'.");
                    }
                }
                else
                {
