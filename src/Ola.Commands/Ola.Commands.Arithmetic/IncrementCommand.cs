...
namespace Ola.Commands.Arithmetic
{
    ...
    {
        private const string RegexPattern = @"^INCREMENT (\w+)$";

        private readonly CommandContext _context;

        public IncrementCommand(CommandContext context)
        {
            _context = context;
        }
            ...
            if (match.Success)
            {
                ...
                if (_context.Variables.ContainsKey(variableName))
                {
                    if (_context.Variables[variableName] is int currentValue)
                    {
                        ...
                        Console.WriteLine($"SUCCESS: Incremented variable '{variableName}' to {_context.Variables[variableName]}");
                    }
