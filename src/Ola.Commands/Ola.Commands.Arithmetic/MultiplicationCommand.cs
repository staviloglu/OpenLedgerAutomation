...
namespace Ola.Commands.Arithmetic
{
    ...
    {
        private const string RegexPattern = @"^MULTIPLY (\w+) BY ([\w\d\.]+) AS (\w+)$";
        private readonly CommandContext _context;

        public MultiplicationCommand(CommandContext context)
        {
            _context = context;
        }
        ...
        public Task<int> Execute(int lineIndex, string[] commands)
        {
            ...
            if (match.Success)
            {
                ...
                if (_context.Variables.ContainsKey(variable1))
                {
                    ...
                    {
                        Console.WriteLine($"ERROR: Cannot multiply incompatible types for variable '{variable1}' and operand '{operand2}'.");
                    }
                }
                else
                {
